...
### 2.3 Spelling Rules
- Words must include the centre letter at least once
- Letters can be reused (including centre)
- Minimum word length: 3 letters
- Valid words checked against dictionary (Steven will provide this once needed)
- We will need to be careful to only present boards that have a decent number of words possible - we can discuss this in the future
...
- **Time bonus per pass:** +20 seconds (100 seconds maximum if all passes completed instantly)
- Time counts down continuously during possession

**Strategic Hold:**
Once the current threshold is reached, the player may:
- Pass immediately (advance to next phase)
- Continue spelling to bank extra points before passing

This creates risk/reward: more time on the current board might make sense if there are still a lot of easy words remaining, but there will be less time for the next board; on the other hand, the next board might be difficult.
...
- [ ] Shot formula: Exact weighting of time vs bonus points
- [ ] Letter distribution: How are letters selected for each ball configuration? (Vowel guarantees? Difficulty tuning?)
...
### Technical
- [ ] Dictionary source and size (affects difficulty significantly)
